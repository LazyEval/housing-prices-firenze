...
# Load model
model = Model.load('models/SVR.pkl')

# Predict
prediction = int(np.round(np.expm1(model.predict(X)), -3)[0])

# Write to front-end
st.write('Estimated price for this house:')
st.write('{} EUR'.format(prediction))
