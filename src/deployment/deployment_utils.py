...
def user_input_features():
    ...
    # District
    values = ['Bellosguardo Galluzzo', 'Campo Di Marte Liberta', 'Centro',
              ...
              'Settignano Rovezzano', 'Ugnano Mantignano',
              'Zona Bolognese Le Cure', 'Zona Firenze Sud']
    ...
    # Type of house
    values = ['appartamento', 'terratetto', 'villa unifamiliare',
              'open space', 'loft', 'attico', 'altro']
    default_ix = values.index('appartamento')
    house_type = st.sidebar.selectbox('Type of house', values,
                                      index=default_ix)

    # Property class
    values = ['economica', 'media', 'signorile', 'lusso']
    default_ix = values.index('media')
    property_class = st.sidebar.selectbox('Property class', values,
                                          index=default_ix)

    # Type of property
    values = ['intera proprietà', 'nuda proprietà', 'a reddito']
    default_ix = values.index('intera proprietà')
    property_type = st.sidebar.selectbox('Property type', values,
                                         index=default_ix)

    # Year of construction
    values = ['(0.0, 1850.0]', '(1850.0, 1950.0]', '(1950.0, 2000.0]',
              '(2000.0, 2021.0]']
    default_ix = values.index('(1950.0, 2000.0]')
    year_of_construction = st.sidebar.selectbox('Year of construction',
                                                values, index=default_ix)

    # Condition
    values = ['ottimo/ristrutturato', 'buono/abitabile', 'da ristrutturare',
    ...
    heating_type = st.sidebar.selectbox('Type of heating', values,
                                        index=default_ix)

    # Heating source
    values = ['metano', 'gas', 'pompa di calore', 'elettrica', 'fotovoltaico',
              'pellet', 'gpl', 'solare']
    default_ix = values.index('metano')
    heating_source = st.sidebar.selectbox('Heating source', values,
                                          index=default_ix)

    # Air conditioning
    values = ['non presente', 'autonomo', 'centralizzato', 'predisposizione']
    default_ix = values.index('non presente')
    air_conditioning = st.sidebar.selectbox('Air conditioning', values,
                                            index=default_ix)

    # Energy efficiency
    values = ['bassa (E, F, G)', 'media (B, C, D)', 'alta (A, A+, A1-A4)']
    default_ix = values.index('bassa (E, F, G)')
    energy_efficiency = st.sidebar.selectbox('Energy efficiency', values,
                                             index=default_ix)
    ...
    values = ['sì', 'no']
    default_ix = values.index('no')
    disabled_access = st.sidebar.selectbox('Disabled access', values,
                                           index=default_ix)
    ...
    values = ['sì', 'no']
    default_ix = values.index('no')
    hydromassage = st.sidebar.selectbox('Hydromassage', values,
                                        index=default_ix)
